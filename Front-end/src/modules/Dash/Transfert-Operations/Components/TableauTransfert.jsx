...
const TableauTransfert = ({ data , Confirm , ChangeFundSource }) => {
  ...
  const LigneDesktop = ({ operation, isLast }) => (
    <tr className={`hidden md:table-row ${!isLast ? 'border-b border-gray-200' : ''}`}>
      ...
      <td className="p-3 text-sm">{operation?.transferToFund?.sku && <>{operation?.fund.name} → {operation?.transferToFund?.name}</> || '-'}</td>
          ...
          <tr className="md:hidden">
            <td colSpan="2" className="p-2 bg-gray-50">
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>{operation?.transferToFund?.sku && <>{operation?.fund.name} → {operation?.transferToFund?.name}</> || '-'}</div>
                ...
                <div>{obtenirLibellestatus(operation.status)==="En attente" ? 
                  <div className='flex flex-col gap-2'>
                    <button id="approve-btn" className="btn-approve bg-black w-fit text-white px-4 py-2 rounded" onClick={()=>Confirm(operation.id)}>
                      Approuver
                    </button>
