...
export const useFetchProductDeleted = () => {
  ...
  const fetchProduct= useCallback(
    async ({ page = 1, limit = 10, fetchAll = false } = {}) => {
      ...
      const url = `${import.meta.env.VITE_BACKEND_URL}/api/products?onlyDeleted=true`;
      ...
      try {
        if (fetchAll) {
          ...
          setTotalProduct(allZones.length);
        } else {
          const response = await axios.get(url, {
        ...
        setError("Une erreur s'est produite lors du chargement des Produits Supprimés.");
      } finally {
        setLoading(false);
      }
    },
